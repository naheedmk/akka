      ...
      def onInternalCommand(cmd: InternalCommand): Behavior[Command] = cmd match {
        ...
        case NodeRemoved(uniqueAddress) =>
          if (uniqueAddress == selfUniqueAddress) {
            ...
            // Ok to update from several nodes but more efficient to try to do it from one node.
            // If cluster.isTerminated we shouldn't try to remove other entries and our own entries should
            // have been removed via watch-Terminated or will be removed by other nodes. This point is
            // anyway too late
            if (isLeader) {
              ctx.log.debug(
